...
from utils.utils import number2string, getDataFile, preProcessDRSBoards,mapDRSChannel2TriggerChannel,findTimeReferenceDelay,denoiseDRS
...
H_ref = 1100
if TRACKFIT:
    fit_goodtracks = np.load(f"root/Run{runNumber}/trackfit_filteredtrack.npz")
    events_goodtracks = fit_goodtracks["event_n"].tolist()
    print("events_goodtracks: ",events_goodtracks)
...
def makeEventDisplays(infilename):
    ...
    for ievt in range(0, rdf.Count().GetValue()):
        ...
        print("evtNumber ", evtNumber)
        if TRACKFIT and evtNumber not in events_goodtracks:
            continue
        ...
        #    continue
        if TRACKFIT:
            coeff = fit_goodtracks["coeff"][events_goodtracks.index(evtNumber)]
            intercept = fit_goodtracks["intercept"][events_goodtracks.index(evtNumber)]
            print("fit: ",f"{coeff}*x+{intercept}")
            fun_fit = ROOT.TF1("fit",f"{coeff}*x+{intercept}",-14,14)
        ...
        if ievt < 100000:
            ...
            DrawHistos([hist2d_Cer, hist2d_Cer_3mm], f"", xmin, xmax, "iX",
                       ...
                       zmin=0.0, zmax=3000.0, doth2=True, W_ref=W_ref, H_ref=H_ref, extraToDraw=[extraToDraw,fun_fit] if TRACKFIT else extraToDraw, extraText="Cer", runNumber=runNumber,
            ...
            DrawHistos([hist2d_Sci, hist2d_Sci_3mm], f"", xmin, xmax, "iX",
                       ...
                       dology=False, drawoptions=["COL,text", "COL,text"],
                       zmin=0.0, zmax=8000.0, doth2=True, W_ref=W_ref, extraToDraw=[extraToDraw,fun_fit] if TRACKFIT else extraToDraw, H_ref=H_ref, extraText="Sci", runNumber=runNumber,
                       outdir=outdir_eventdisplay)
    ...
    """
        ...
        for _, DRSBoard in DRSBoards.items():
            for iTowerX, iTowerY in DRSBoard.GetListOfTowers():
                ...
                for var in ["Cer", "Sci"]:
                    ...
                    pulse_shape = denoiseDRS(pulse_shape)
                    triggername = mapDRSChannel2TriggerChannel(varname)
                    trigger_shape = rdf.Take["ROOT::VecOps::RVec<float>"](
                        triggername).GetValue()[ievt]
                    trigger_time = ROOT.fit_riseT(trigger_shape,0.5,2,-1,0, 200,  600, 1000,False,False,"")-findTimeReferenceDelay(triggername)
                    ...
                    for i in range(len(pulse_shape)):
                        print("i-trigger_time+1024: ",i-trigger_time+1024)
                        h1.Fill(i-trigger_time+1024, pulse_shape[i])
        ...
        for (sTowerX, sTowerY) in hists_pulse_shapes:
            ...
            extraToDraw.SetTextSize(0.04)
            extraToDraw.AddText(
                f"Event: {evtNumber}, iTowerX: {sTowerX}, iTowerY: {sTowerY}")
            ...
            DrawHistos(to_draws, labels, 0, 1024, "TS-trigger+1024",
                       -5, 30, "Amplitude",
...
if __name__ == "__main__":
    input_file = f"root/Run{runNumber}/filtered_highE.root"
